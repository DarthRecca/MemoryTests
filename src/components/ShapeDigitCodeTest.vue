            ...
            <v-container>
                <v-row>
                    <v-col class="progress-timer" cols="10">
                        <v-progress-linear :model-value="this.timer" color="purple" height="40"></v-progress-linear>
                    </v-col>
                    <v-col class="score-text" cols="auto">
                        Your Score
                    </v-col>
            ...
            <v-row>
                ...
                <v-col>
                    <div class="shape-digit-code-prompt">
                        <p>
                            <v-icon size="large" color="black" :icon="this.promptShape" class="prompt-icon"></v-icon>
                        </p>
                    </div>
                </v-col>
                <v-col>
        ...
        <div class="results">
            <center>
                <table>
                    <tr>
                        <td>
                            Number of Correct Digits Coded
                        </td>
                        <td>
                            {{ this.score }}
                        </td>
...
export default {
    ...
    methods: {
        ...
        generatePrompt() {
            const randomIndex = Math.floor(Math.random() * this.keyDigits.length);
            this.promptDigit = this.keyDigits[randomIndex]
            this.promptShape = this.keyShapes[randomIndex];
        },
        testCompleted() {
            useTestStore().addShapeDigitCodeTestData({
                shapeDigitCodeTestScore: this.score
            });
            this.completed = true
        }
    },
