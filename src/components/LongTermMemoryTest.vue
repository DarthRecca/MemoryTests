<template>
    <v-container>
        <div v-if="!this.completed">
            <p>
                Recall the words you were shown in the previous screen and enter in following boxes one
                by one.<br /> You can enter in any order (sequence).<br /> Try entering them as accurately as possible
                but a minor spelling mistake will be accommodated.<br />
            </p>
            <v-row>
                <v-col>
    ...
    methods: {
        checkKeys() {
            ...
            for (let i = 0; i < inputKeyList.length; i++) {
                if (this.answerKeys.includes(inputKeyList[i])) {
                    this.keysMatched += 1
                }
            }
            this.completed = true
